...
async function connectToDatabase() {
  try {
    const uri = 'mongodb+srv://sheshadrimbsc22:uniquesheshadri@cluster0.4vsx93w.mongodb.net/shopping?retryWrites=true&w=majority'; // Update with your MongoDB connection URI
...
async function saveShopsToDatabase(shops, collection) {
  try {
...
async function readShopsFromDatabase(collection) {
  try {
...
async function updateShopInDatabase(collection, shop) {
  try {
    await collection.updateOne({ name: shop.name }, { $set: { rent: shop.rent } });
...
async function deleteShopFromDatabase(collection, shopName) {
  try {
    await collection.deleteOne({ name: shopName });
...
async function main() {
  ...
  const database = client.db('shopping'); // Replace 'shopping' with your database name
  const collection = database.collection('shops');

  await saveShopsToDatabase(sumOfNShopRents.shops, collection);

  const shopsFromDB = await readShopsFromDatabase(collection);
  ...
  switch (operation.toUpperCase()) {
    case 'C':
      ...
      const newShop = new Shop(shopName, rent);
      await saveShopsToDatabase([newShop], collection);
      break;
    case 'R':
      const retrievedShops = await readShopsFromDatabase(collection);
    ...
    case 'U':
      ...
      if (shopToUpdate) {
        ...
        await updateShopInDatabase(collection, shopToUpdate);
      } else {
    ...
    case 'D':
      ...
      await deleteShopFromDatabase(collection, shopToDeleteName);
      break;
    default:
